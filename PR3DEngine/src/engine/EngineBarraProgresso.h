...
*/
class EngineBarraProgresso : public ResourceGroupListener
{
protected:
...
public:
	EngineBarraProgresso() {}
	virtual ~EngineBarraProgresso(){}
		...
		Real initProportion = 0.70f)
	{
		...
		if (!mLoadOverlay)
		{
			OGRE_EXCEPT(Exception::ERR_ITEM_NOT_FOUND, 
				"Nao foi possivel encontrar: loading overlay", "EngineBarraProgresso::start");
		}
		mLoadOverlay->show();
	...
	*/
	...
	void resourceGroupScriptingStarted(const String& groupName, size_t scriptCount)
	{
		...
		mLoadingDescriptionElement->setCaption("Processando scripts...");
		_window->update();
	}
	...
	void resourceGroupLoadStarted(const String& groupName, size_t resourceCount)
	{
		...
		mLoadingDescriptionElement->setCaption("Carregando recursos...");
		_window->update();
	}
